...
def active_selection(model, unlabelled_data, nb_data, active_method, threshold=0.05):
...
def qbc_selection(model, unlabelled_data, nb_data):
    ...
    subset_index = np.random.permutation(len(unlabelled_data[0]))
    subset = unlabelled_data[0][subset_index[:n]]
    qbdc_index = np.argsort(disagreement_coeff(subset, model, f=build_model))[::-1] # high disagreement
...
if __name__=="__main__":
    ...
    parser.add_argument('--filename', type=str, default='svhn_ceal', help='csv filename')
    parser.add_argument('--tmp_save', type=str, default='svhn_ceal_saving', help='csv filename')
